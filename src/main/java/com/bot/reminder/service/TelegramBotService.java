...
public class TelegramBotService extends TelegramLongPollingBot {
    ...
    private void handleDaftar(Long chatId) {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            if ("TUGAS".equalsIgnoreCase(task.getCategory())) {
                response.append("   ⏰ <b>Deadline:</b> ").append(formatIndonesianDateTime(task.getDeadlineTime())).append("\n");
            } else {
                response.append("   ⏰ <b>Waktu:</b> ").append(formatIndonesianDateTime(task.getReminderTime()));
    ...
    private void handleSetLokasi(Long chatId, String messageText) {
        ...
        if (query.matches("\\d+")) {
            ...
            if (cityName != null) {
                UserSettings settings = new UserSettings(chatId, query, cityName, LocalDateTime.now());
            ...
            if (cities.isEmpty()) {
                ...
                UserSettings settings = new UserSettings(chatId, cityId, cityName, LocalDateTime.now());
                userSettingsRepository.save(settings);
    ...
    private void handleJadwal(Long chatId) {
        ...
        if (settingsOpt.isEmpty()) {
            ...
            if (times.isEmpty()) {
                ...
                response.append("• Subuh: <b>").append(times.get("subuh")).append(" WIB</b>\n")
                        .append("• Dzuhur: <b>").append(times.get("dzuhur")).append(" WIB</b>\n")
                        .append("• Ashar: <b>").append(times.get("ashar")).append(" WIB</b>\n")
                        .append("• Maghrib: <b>").append(times.get("maghrib")).append(" WIB</b>\n")
                        .append("• Isya: <b>").append(times.get("isya")).append(" WIB</b>\n\n");
            }
        }
        ...
        if (deadlineTasks.isEmpty()) {
            ...
            for (int i = 0; i < deadlineTasks.size(); i++) {
                ...
                response.append(i + 1).append(". <b>").append(task.getDescription()).append("</b>\n")
                        .append("   ⏰ Deadline: ").append(formatIndonesianDateTime(task.getDeadlineTime())).append("\n");
                if (task.getNotes() != null && !task.getNotes().trim().isEmpty()) {
                    response.append("   ℹ️ <i>Catatan: ").append(task.getNotes()).append("</i>\n");
                }
                response.append("\n");
        ...
        if (otherReminders.isEmpty()) {
            ...
            for (int i = 0; i < otherReminders.size(); i++) {
                ...
                response.append(i + 1).append(". ").append(catEmoji).append(" <b>").append(task.getDescription()).append("</b>\n")
                        .append("   ⏰ Waktu: ").append(formatIndonesianDateTime(task.getReminderTime()));
                if (!"NONE".equalsIgnoreCase(task.getRecurrence())) {
                    response.append(" (").append(getRecurrenceLabel(task.getRecurrence())).append(")");
                }
                response.append("\n");
    ...
    private void handleNaturalLanguageTask(Long chatId, String messageText) {
        ...
        try {
            ...
            LocalDateTime targetTime = LocalDateTime.parse(datetimeStr, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
            ...
            if ("TUGAS".equalsIgnoreCase(task.getCategory())) {
                ...
                LocalDateTime tH3 = targetTime.minusDays(3);
                if (tH3.isAfter(now)) {
                    task.addReminder(new Reminder(task, tH3, "H3", null));
                    scheduledRemindersCount++;
                }

                LocalDateTime t3H = targetTime.minusHours(3);
                LocalDate startDay = (tH3.isBefore(now) ? now : tH3).toLocalDate();
                LocalDate endDay = targetTime.toLocalDate();

                for (LocalDate date = startDay; !date.isAfter(endDay); date = date.plusDays(1)) {
                    LocalDateTime morningTime = LocalDateTime.of(date, LocalTime.of(9, 0));
                    LocalDateTime eveningTime = LocalDateTime.of(date, LocalTime.of(19, 0));

                    if (morningTime.isAfter(now) && morningTime.isBefore(t3H) && (morningTime.isAfter(tH3) || morningTime.isEqual(tH3))) {
                        task.addReminder(new Reminder(task, morningTime, "DAILY", null));
                        scheduledRemindersCount++;
                    }

                    if (eveningTime.isAfter(now) && eveningTime.isBefore(t3H) && (eveningTime.isAfter(tH3) || eveningTime.isEqual(tH3))) {
                        task.addReminder(new Reminder(task, eveningTime, "DAILY", null));
                        scheduledRemindersCount++;
                    }
                }

                if (t3H.isAfter(now)) {
                    task.addReminder(new Reminder(task, t3H, "THREE_HOURS", null));
                    scheduledRemindersCount++;
                }
            ...
            if ("TUGAS".equalsIgnoreCase(task.getCategory())) {
                successMessage.append("⏰ <b>Deadline:</b> ").append(formatIndonesianDateTime(targetTime)).append("\n")
                               .append("🔔 <b>Total Pengingat Terjadwal:</b> ").append(scheduledRemindersCount).append(" notifikasi (H-3, harian 9:00/19:00, & H-3 jam)");
            } else {
                successMessage.append("⏰ <b>Waktu:</b> ").append(formatIndonesianDateTime(targetTime));
            }
